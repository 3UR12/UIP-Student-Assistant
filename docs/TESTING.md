...
9. From an empty dashboard, wait for Moodle to load the course list and confirm it navigates to `/my/` once. During **Actualizar materias**, the existing list must remain visible; if Moodle requests login or times out, use the visible **Abrir Moodle** or **Reintentar** action instead of expecting automatic retries.
