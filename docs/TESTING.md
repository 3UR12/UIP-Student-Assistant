...
`core-smoke.test.js` retains scanner regression checks, including visible Moodle warning extraction. `my-courses-discovery.test.js` verifies My Courses recognition, multi-scope extraction, valid cards in an aside, navigation exclusions, document fallback, and safe counters. The retired v0.4 popup-hydration test was intentionally removed because popup orchestration no longer exists. `automation-engine.test.js` covers state transitions, login, timeout, pause, cancel, duplicate-submission protection, activity metadata, and storage sanitization. `workflow-service.test.js` covers the MV3 message boundary, dedicated-worker creation instead of redirecting a user Moodle tab, discovery, exclusion of unnamed general sections, persisted run creation, alarms, dashboard closure, worker-tab rebinding, and duplicate page-ready/tab events with a deterministic Chrome mock. `dashboard-ux.test.js` enforces the dashboard's visible state/control contract. `dashboard-modules-ready.test.js` reproduces the real 15-module setup, confirmation, start flow, and terminal 15-module outcome summary after `modules-ready`. `dashboard-start-lifecycle.test.js` verifies a second run after DONE, start-failure retry, double-start prevention, dashboard reopen, and 20 consecutive DONE → Nuevo recorrido → START cycles. `terminal-outcomes.test.js` verifies terminal outcome semantics, safe per-module reasons, late Feedback/Form hydration, and the repeated-issue circuit breaker. `automation-e2e.test.js --repeat=20` simulates a two-module fully automatic run twenty times, including a worker-tab close/rebind/resume run. `lifecycle-idempotency.test.js` covers restart boundaries, duplicate events, worker loss, login recovery, and bounded watchdog behavior. `recovery-service-worker.test.js` proves that a restarted service worker re-arms its watchdog, survives a missing initial scan, and reuses an already-open dashboard tab. `event-pump-race.test.js --repeat=50` reproduces post-submit Moodle completion arriving while prefill/submit is still active, with synthetic jitter and no watchdog recovery. `blocked-section-redirect.test.js --repeat=50` simulates course 8169 redirecting Módulo #2 to the course with “no disponible”, verifies that the module is blocked and omitted, confirms same-DOM coalescing only within one transition, and asserts the two-navigation recovery cap. `discovery-lifecycle.test.js --repeat=50` simulates primary My Courses discovery from arbitrary Moodle pages, dedicated-tab bootstrap, async cards, automatic post-login resume, a one-time dashboard fallback after a terminal empty primary scan, stale observers, stale-while-revalidate, and timeout. `discovery-settlement-content.test.js` proves the bounded content-script `MutationObserver` emits a terminal discovery event only after async cards appear.
