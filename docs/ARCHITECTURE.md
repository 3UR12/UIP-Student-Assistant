...
`extension/background/workflow-service.js` is the Manifest V3 owner. It persists allow-listed state in `chrome.storage.session`, owns a dedicated Moodle worker tab, schedules the watchdog alarm, serializes DOM effects, and exposes the dashboard message API. A read-after-write check protects every workflow-state transition. It reuses only a persisted worker tab, never an arbitrary Moodle tab the student may be reading.
...
The workflow stores version, run ID, status/phase, worker tab ID, canonical course/module IDs and URLs, selected rating, module states, aggregate counts, safe last-event/retry metadata, a step timestamp, and a bounded activity log. Each log item contains only timestamp, safe action label, module/Feedback IDs, and observed names. It intentionally excludes DOM fragments, hidden fields, form values, submission bodies, tokens, cookies, diagnostics, and credentials. When Moodle shows its normal login page, discovery remains explicit about the required sign-in and automatically resumes My Courses scanning after Moodle reports a logged-in page.
