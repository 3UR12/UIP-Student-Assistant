...
`extension/content/content.js` is a thin, passive integration layer. It scans only after the popup sends an explicit message. `extension/popup/` presents the result and copies only an allow-listed diagnostic. v0.1.2 does not need a background service worker.
