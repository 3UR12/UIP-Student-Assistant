...
7. After Moodle navigates, the student reopens the popup and scans again. Completion is verified only from post-submit DOM evidence. Any Continue/return link and any section previous/next relation are read from real visible Moodle links; Continue requires a separate explicit action and no multi-module loop is implemented.
