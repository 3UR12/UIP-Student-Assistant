(() => {
  ...
  function renderProgress(workflow) {
    ...
    ui.progressLabel.textContent = `Procesando ${Math.min(reviewed + 1, total)} de ${total} módulos · ${percent} % · ${reviewed} evaluados`;
  ...
  function renderDone(workflow) {
    ...
    const hasIssues = workflow.terminalOutcome === "COMPLETED_WITH_ISSUES" || (progress.manualRequired || 0) > 0 || (progress.failed || 0) > 0;
    $("#done-title").textContent = hasIssues ? "Recorrido finalizado con incidencias" : "Recorrido completado";
    $("#done-summary").textContent = hasIssues ? `${progress.reviewed || 0} de ${progress.total || 0} módulos fueron evaluados por el recorrido. Algunos módulos requieren revisión.` : `${progress.reviewed || 0} de ${progress.total || 0} módulos fueron evaluados por el recorrido.`;
    $("#done-submitted").textContent = progress.submitted || 0; $("#done-completed").textContent = progress.alreadyCompleted || 0; $("#done-empty").textContent = progress.noFeedback || 0; $("#done-blocked").textContent = progress.skipped || 0; $("#done-manual").textContent = progress.manualRequired || 0; $("#done-failed").textContent = progress.failed || 0;
    const outcomes = workflow.moduleOutcomes || workflow.modules || [];
    $("#done-module-summary").innerHTML = outcomes.map((item) => `<li class="module-status ${escape(item.status)}"><span>${item.status === "completed" ? "✓" : item.status === "blocked" ? "–" : item.status === "manual-required" || item.status === "failed" ? "!" : "○"}</span><strong>${escape(item.name || `Módulo #${item.id}`)}</strong><em>${escape(item.status === "completed" ? "Completado" : item.status === "blocked" ? "No disponible" : item.status === "manual-required" ? "Revisión manual" : item.status === "failed" ? "Fallido" : item.status === "no-feedback" ? "Sin Feedback" : "Evaluado")}</em>${outcomeLabel(item.reason) ? `<small>${escape(outcomeLabel(item.reason))}</small>` : ""}</li>`).join("");
  }
  function render() {
