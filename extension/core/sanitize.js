...
(function attachSanitizer(global) {
  ...
  core.sanitizeDiagnostic = function sanitizeDiagnostic(scan) {
    ...
    return {
      ...
      feedbackResult: scan.feedbackResult ? { feedbackId: scan.feedbackResult.feedbackId || null, state: ["completed", "confirmation", "still-editable", "unknown"].includes(scan.feedbackResult.state) ? scan.feedbackResult.state : "unknown", completionState: completion(scan.feedbackResult.completionState), submissionVerified: maybeBoolean(scan.feedbackResult.submissionVerified), continueAction: scan.feedbackResult.continueAction ? { detected: scan.feedbackResult.continueAction.detected === true, unique: scan.feedbackResult.continueAction.unique === true, url: cleanUrl(scan.feedbackResult.continueAction.url), label: cleanLabel(scan.feedbackResult.continueAction.label) } : null } : null,
